...
namespace TopSoSanh.Services.Implement
{
	public class CrawlDataCommon : ICrawlDataCommon
	{
		...
		public List<PriceCompare> GetPriceOtherShop(string productName, Shop shop)
		{
			List<PriceCompare> priceCompares = new List<PriceCompare>();
			List<Task> tasks = new List<Task>();
			if (shop != Shop.Anphat)
				tasks.Add(Task.Run(() => _crawlDataAnphatService.GetPriceByName(productName, priceCompares)));
			if (shop != Shop.Ankhang)
				tasks.Add(Task.Run(() => _crawlDataAnkhangService.GetPriceByName(productName, priceCompares)));
			if (shop != Shop.Gearvn)
				tasks.Add(Task.Run(() => _crawlDataGearvnService.GetPriceByName(productName, priceCompares)));
			Task.WaitAll(tasks.ToArray());
			return priceCompares;
		}
